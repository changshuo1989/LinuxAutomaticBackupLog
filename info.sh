...
if [ "`$(which diff) ${DIR}/${FILE} ${TRIGGER_DIR}/${TRIGGER_FILE}`" = "" ] && [ "$FILE" == "$TRIGGER_FILE" ]; then
	...
	if [ "$NAME" != "" ] && [ "$START_TIME" != "" ] && [ "$END_TIME" != "" ] && [ "$ELAPSED_TIME" != "" ] && [ "$ERRORS" != "" ] && [ "$DESTINATION" != '*' ]; then
		#send curl request
		$(which curl) --data "request=addbackuphistory&type=${NAME}&starttime=${START_TIME}&endtime=${END_TIME}&elapsedtime=${ELAPSED_TIME}&errors=${ERRORS}&interval=${INTERVAL}" ${DESTINATION}
	fi
